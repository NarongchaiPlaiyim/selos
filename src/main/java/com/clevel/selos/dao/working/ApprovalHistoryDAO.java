...
public class ApprovalHistoryDAO extends GenericDAO<ApprovalHistory, Long> {
    ...
    public ApprovalHistory findExistHistoryByUser(long workCaseId, String userId) {
        Criteria criteria = createCriteria();
        criteria.add(Restrictions.eq("workCase.id", workCaseId));
    ...
    public ApprovalHistory findExistHistoryByApprovalType(long workCaseId, String userId, int approveType){
        Criteria criteria = createCriteria();
        criteria.add(Restrictions.eq("workCase.id", workCaseId));
        criteria.add(Restrictions.eq("user.id", userId));
    ...
    public ApprovalHistory findByWorkCaseAndUser(long workCaseId, String userId) {
        Criteria criteria = createCriteria();
        criteria.add(Restrictions.eq("workCase.id", workCaseId));
        criteria.add(Restrictions.eq("isSubmit", 0));
        criteria.add(Restrictions.eq("user.id", userId));
