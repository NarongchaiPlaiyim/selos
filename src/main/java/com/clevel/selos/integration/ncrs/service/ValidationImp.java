...
public class ValidationImp implements Validation {
    ...
    public void validation(NCRSModel model) throws Exception {
        if (null==model){
            throw new ValidationException(message.get("validation.101"));
        }
        ...
        if(!"01".equals(model.getEnqpurpose())||!"02".equals(model.getEnqpurpose())){
            throw new ValidationException(message.get("validation.105"));
        }
        ...
        if(!"Y".equals(model.getEnqpurpose())){
            throw new ValidationException(message.get("validation.107"));
        }
