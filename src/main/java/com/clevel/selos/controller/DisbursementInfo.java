...
public class DisbursementInfo implements Serializable {
	...
	private void _loadDropdown() {
		...
		/*
		 * for (long i=0; i<=5; i++){ SelectItem item = new SelectItem();
		 * item.setValue(i); item.setLabel(i+"99-9-99999-9");
		 * item.setDescription(i+"_accountName"); log.debug("accountList: " +
		 * i); accountList.add(item); }
		 */
		// }
	}
	...
	public Date getLastUpdateDateTime() {
		if (disbursementInfoView.getModifyDate() != null) {
	...
	public void updateAccountName() {
		...
		if (disbursementDepositDetailView.getOpenAccountId() > 0) {
			...
			disbursementDepositDetailView.setDisbursementCreditTypeView(disbursementControl
					.getDisbursementCreditViewByOpenAccountId(disbursementDepositDetailView.getOpenAccountId()));
		}
	}
	...
	public void onOpenEditDepositDialog() {
		...
		modeForButton = ModeForButton.EDIT;
		// disbursementDepositDetailView = new
		// DisbursementDepositBaDetailView();
		...
		if (modeForButton != null && modeForButton.equals(ModeForButton.ADD)) {
			...
			if (disbursementMcDetailView != null || disbursementMcDetailView.getDisbursementCreditTypeView() != null) {
				...
				for (int i = 0; i < disbursementMcDetailView.getDisbursementCreditTypeView().size(); i++) {
					if (disbursementMcDetailView.getDisbursementCreditTypeView().get(i).getDisburseAmount()
							.compareTo(disbursementMcDetailView.getDisbursementCreditTypeView().get(i).getLimitAmount()) > 0) {
		...
		if (modeForButton != null && modeForButton.equals(ModeForButton.EDIT)) {
			...
			if (disbursementMcDetailView != null || disbursementMcDetailView.getDisbursementCreditTypeView() != null) {
				...
				if (falseCount == 0) {
					...
					this.disbursementInfoView.getDisburseMcList().get(selectRowNumber)
							.setIssuedDate(disbursementMcDetailView.getIssuedDate());
					this.disbursementInfoView.getDisburseMcList().get(selectRowNumber)
							.setPayeeName(disbursementMcDetailView.getPayeeName());
					this.disbursementInfoView.getDisburseMcList().get(selectRowNumber)
							.setPayeeSubname(disbursementMcDetailView.getPayeeSubname());
					this.disbursementInfoView.getDisburseMcList().get(selectRowNumber)
							.setCrossType(disbursementMcDetailView.getCrossType());
					this.disbursementInfoView.getDisburseMcList().get(selectRowNumber)
							.setDisbursementCreditTypeView(disbursementMcDetailView.getDisbursementCreditTypeView());
	...
	public void onSubmitDisbursementBahtnet() {
		...
		if (modeForButton != null && modeForButton.equals(ModeForButton.ADD)) {
			...
			if (disbursementBahtnetDetailView != null || disbursementBahtnetDetailView.getDisbursementCreditTypeView() != null) {
				...
				for (int i = 0; i < disbursementBahtnetDetailView.getDisbursementCreditTypeView().size(); i++) {
					if (disbursementBahtnetDetailView.getDisbursementCreditTypeView().get(i).getDisburseAmount()
							.compareTo(disbursementBahtnetDetailView.getDisbursementCreditTypeView().get(i).getLimitAmount()) > 0) {
		...
		if (modeForButton != null && modeForButton.equals(ModeForButton.EDIT)) {
			...
			if (disbursementBahtnetDetailView != null || disbursementBahtnetDetailView.getDisbursementCreditTypeView() != null) {
				...
				if (falseCount == 0) {
					this.disbursementInfoView.getDisbursementBahtnetList().get(selectRowNumber)
							.setBankCode(disbursementBahtnetDetailView.getBankCode());
					this.disbursementInfoView.getDisbursementBahtnetList().get(selectRowNumber)
							.setBenefitName(disbursementBahtnetDetailView.getBenefitName());
					this.disbursementInfoView.getDisbursementBahtnetList().get(selectRowNumber)
							.setAccountNumber(disbursementBahtnetDetailView.getAccountNumber());
					this.disbursementInfoView.getDisbursementBahtnetList().get(selectRowNumber)
							.setBranchName(disbursementBahtnetDetailView.getBranchName());
					this.disbursementInfoView.getDisbursementBahtnetList().get(selectRowNumber)
							.setDisbursementCreditTypeView(disbursementBahtnetDetailView.getDisbursementCreditTypeView());
	...
	private void calculationSummary() {
		...
		for (DisbursementSummaryView disbursementSummaryView : this.disbursementInfoView.getDisburse()) {
			...
			disbursementSummaryView.setDiffAmount(disbursementSummaryView.getApprovedLimit()
					.subtract(disbursementSummaryView.getHoldAmount()).subtract(totalDisburse));
		}
	}
	...
	private void calculationSummaryTotalMc() {
		...
		if (getDisbursementInfoView().getDisburseMcList() != null) {
			for (int i = 0; i < listSize; i++) {
				for (DisbursementCreditTypeView disbursementCreditTypeView : getDisbursementInfoView().getDisburseMcList().get(i)
						.getDisbursementCreditTypeView()) {
					...
					if (this.totalDisburseMCAmount.get(newCreditDetailId) != null)
						this.totalDisburseMCAmount.put(newCreditDetailId,
								disburseAmount.add(this.totalDisburseMCAmount.get(newCreditDetailId)));
					else
	...
	private void calculationSummaryTotalDeposit() {
		...
		if (getDisbursementInfoView().getDisburseDepositList() != null) {
			for (int i = 0; i < listSize; i++) {
				for (DisbursementCreditTypeView disbursementCreditTypeView : getDisbursementInfoView().getDisburseDepositList().get(i)
						.getDisbursementCreditTypeView()) {
					long newCreditDetailId = disbursementCreditTypeView.getNewCreditDetailId();
					BigDecimal disburseAmount = disbursementCreditTypeView.getDisburseAmount();
					if (this.totalDisburseTRAmount.get(newCreditDetailId) != null)
						this.totalDisburseTRAmount.put(newCreditDetailId,
								disburseAmount.add(this.totalDisburseTRAmount.get(newCreditDetailId)));
					else
						this.totalDisburseTRAmount.put(newCreditDetailId, disburseAmount);
				}
				totalDepositDisbursement = totalDepositDisbursement.add(getDisbursementInfoView().getDisburseDepositList().get(i)
						.getTotalAmount());
			}
		}
	...
	private void calculationSummaryTotalBahtnet() {
		...
		if (getDisbursementInfoView().getDisbursementBahtnetList() != null) {
			for (int i = 0; i < listSize; i++) {
				for (DisbursementCreditTypeView disbursementCreditTypeView : getDisbursementInfoView().getDisbursementBahtnetList().get(i)
						.getDisbursementCreditTypeView()) {
					long newCreditDetailId = disbursementCreditTypeView.getNewCreditDetailId();
					BigDecimal disburseAmount = disbursementCreditTypeView.getDisburseAmount();
					if (this.totalDisburseBAAmount.get(newCreditDetailId) != null)
						this.totalDisburseBAAmount.put(newCreditDetailId,
								disburseAmount.add(this.totalDisburseBAAmount.get(newCreditDetailId)));
					else
						this.totalDisburseBAAmount.put(newCreditDetailId, disburseAmount);
				}
				totalBahtnetDisbursement = totalBahtnetDisbursement.add(getDisbursementInfoView().getDisbursementBahtnetList().get(i)
						.getTotalAmount());
			}
		}
