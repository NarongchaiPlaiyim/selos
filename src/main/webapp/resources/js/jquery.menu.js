...
buildmenu: function($, setting){
	...
	if(method === 'toggle') {
		...
		$(window).resize(function(){
			...
			shadowstimer = setTimeout(function(){$shadows.removeClass('selosmenushadowsnone');}, 100);
		});
	}
...
init: function(setting){
	if(this.detectie6 && parseFloat(jQuery.fn.jquery) > 1.3){
		this.init = function(setting){
			if (typeof setting.contentsource=="object"){ //if external ajax menu
				jQuery(function($){selosmenu.getajaxmenu($, setting, 'nobuild');});
			}
			return false;
		};
	...
	jQuery(document).ready(function($){ //ajax menu?
		if (setting.shadow && selosmenu.css3support){$('body').addClass('ddcss3support');}
		if (typeof setting.contentsource=="object"){ //if external ajax menu
...
if(selosmenu.detectie9){
	(function($){ //begin Scripted CSS Patch
		...
		selosmenu.ie9shadow = function($elem){ //runs once
			...
			'</style>\n'].join('\n'));
			selosmenu.ie9shadow = function(){}; //becomes empty function after running once
